...
### 1.2 Goals
- [ ] Support dynamic target URLs (e.g., preview/staging URLs passed via environment variables).
- [ ] Crawl the target site to map out pages, routes, navigation elements, and check for basic load errors (4xx/5xx).
- [ ] Support automated, authenticated E2E flows using dynamically supplied test user credentials.
- [ ] Perform transaction/checkout validation using Stripe test credit cards.
- [ ] Re-run the tests dynamically on demand, identify broken selectors, and compile a final QA execution report.
...
- [ ] The crawler script maps all available routes on the target URL.
- [ ] The E2E tests execute successfully against any live environment given its URL and test credentials.
- [ ] Login, dashboard navigation, and Stripe checkout (test mode) succeed and pass assertions.
- [ ] Failing selectors generate readable error traces, allowing immediate pinpointing of DOM issues.
- [ ] A final QA report compiles all results and logs them.
...
|----------|--------|--------|
| Should the crawler automatically *generate* test files, or should we have static, highly parameterized test suites that adapt to whatever base URL is given? | Open | To be analyzed in Phase 1. Parameterized static suites are typically more robust, but a crawler is useful for link checking. |
| How do we handle database seeding for dynamic target URLs (since we cannot run local Prisma pushes on a remote staging environment)? | Open | E2E tests must either rely on pre-existing users or register new random test users dynamically. |
