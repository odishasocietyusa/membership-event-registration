...
- Admins can create, update, delete award records and upload an award photo to Supabase Storage (`award-photos` bucket); the public URL is persisted to `awards.photo_url`.
...
3. Provide a separate photo-upload endpoint that stores the file in Supabase Storage and writes the resulting public URL into `photo_url`.
...
| REQ-03 | Admin `POST /api/awards` creates an award record                            | Functional    | Med        | `withAuth(admin)`, `award_names` seed |
| REQ-04 | Admin `PUT /api/awards/:id` updates an award record                         | Functional    | Med        | REQ-03              |
| REQ-05 | Admin `DELETE /api/awards/:id` deletes an award record; returns 204         | Functional    | Low        | REQ-03              |
| REQ-06 | Admin `POST /api/awards/:id/photo` uploads file → Supabase Storage → URL    | Functional    | Med        | Supabase Storage SDK, `award-photos` bucket |
...
| REQ-10 | Photos must NOT be stored as base64 in DB                                   | NonFunctional | Low        | REQ-06              |

### 2.2 Implicit Requirements
- [x] Zod validation for create/update payloads and the `year`/`category` query params.
...
- [x] Photo upload endpoint accepts `multipart/form-data` (file field) — this is the standard primitive for browser uploads and avoids base64.
...
6. **Photo upload on non-existent award id** → 404 (validate before uploading to avoid orphaned storage objects).
7. **Photo upload with unsupported MIME or no file** → 400.
8. **Updating photo replaces URL but old object remains in Storage** — out of scope for cleanup; acceptable (spec mentions no deletion-cascade requirement).
9. **`recipient_member_id` references soft-deleted member** → allow (historical award shouldn't disappear because member was deactivated).
...
- Routes: `app/api/awards/route.ts`, `app/api/awards/[id]/route.ts`, `app/api/awards/[id]/photo/route.ts`.
...
- Storage object cleanup on photo replace / award delete.
...
- A4. **Photo upload contract** — `multipart/form-data` with field `file`. Accepts `image/png`, `image/jpeg`, `image/webp`. Max 5MB (defensive, not in spec). Object stored under `awards/{awardId}/{timestamp}-{filename}` in `award-photos` bucket; bucket assumed already configured as public.
...
|---------------------------------------------------------------|------------|--------|----------------------------------------------------------------------------|
| Supabase Storage bucket `award-photos` not provisioned         | Med        | Med    | Document setup in implementation notes; service throws a clear error if the upload call returns a bucket-not-found error. |
...
| Photo replace leaves orphan objects in Storage                 | Med        | Low    | Out of scope; documented.                                                  |
...
| Multipart body parsing in Next.js App Router edge cases        | Low        | Med    | Use `req.formData()` on Node runtime; reject non-multipart with 400.       |
...
4. Photo upload is a separate route (`POST /api/awards/:id/photo`), uses `multipart/form-data`, uploads via `supabaseAdmin.storage.from('award-photos').upload(...)`, then writes the public URL into `awards.photo_url`.
