...
### 1.1 Summary
A member-facing search page at `/members/search` that lets any authenticated, active OSA member search the membership directory by name, city, state, and country. The search form has a single **Name** field that matches full name, partial name, first-name-only, or last-name-only against a member's own name *and* their spouse's name (children and other family relations are never matched). City, State, and Country act as additional filters. Results are displayed in an unstyled table showing Name, Spouse, City, State, Member Since, Membership Type, and Membership Status. Each result row includes a "Send Message" link pre-encoded with the target member's ID for future messaging epic wiring. The searcher must be active; results include members of any status (active, expired, suspended) so members can reconnect regardless of their peer's standing.

> **Note:** This spec was originally implemented with separate First Name / Last Name fields and a 100-per-page result size. It was later revised (2026-06-07) to (a) consolidate First/Last Name into a single Name field that also matches against spouse names, and (b) reduce the page size to 25. The requirements below reflect the current, revised design.
...
- [ ] Search supports four filter fields: Name (single field), City, State (dropdown), Country (dropdown: USA / Canada)
- [ ] Name field matches full name, partial name, first-name-only, or last-name-only via case-insensitive substring match against the member's own `fullName` AND the `fullName` of any linked spouse `FamilyMember` (relation = `spouse`); child and other family relations are never matched
- [ ] State dropdown shows US states by default; swaps to Canadian provinces when Canada is selected
- [ ] Blank/trivial searches are rejected: the Name or City field must have ≥ 3 characters, OR a state must be selected; country alone is never sufficient
- [ ] Results include members of any status (active, expired, suspended) — not restricted to active only
- [ ] Results paginate at 25 per page with total count displayed; hard cap of 1 000 total results
- [ ] Results table shows seven columns: Name, Spouse, City, State, Member Since, Membership Type, Membership Status
- [ ] Spouse column shows the spouse's full name, or `-NA-` when the member has no spouse on file
...
- Matching the Name field against children's names or other non-spouse family members
- Respecting per-member `profileVisibility` flags for result columns — deferred to a separate epic
- Sorting controls in the UI (results ordered by name ascending)
...
| FR-03 | Search form has a single Name text input plus City, Country dropdown, and State/Province dropdown | Must Have | Country defaults to USA; State defaults to empty |
...
| FR-06 | Search is rejected client-side if: Name is empty AND City is empty AND no state is selected | Must Have | Country alone is never a valid sole filter |
| FR-07 | The Name or City text input, if non-empty, must contain ≥ 3 characters; shorter values show a validation error and block submission | Must Have | Client-side guard |
...
| FR-11 | The Name filter uses case-insensitive substring match against the member's own `fullName` OR the `fullName` of a linked spouse `FamilyMember` (relation = `spouse`, not soft-deleted); a single query value covers full name, partial name, first-name-only, and last-name-only | Must Have | Children and other family relations are excluded from matching |
...
| FR-15 | API response includes `total` (capped at 1 000), `page`, `pageSize` (25), and `results` array | Must Have | Total reflects matched records up to the 1 000 cap |
...
| FR-17 | Page UI shows total result count and current page range (e.g. "Showing 1–25 of 243 results") | Must Have | |
| FR-18 | Pagination controls navigate between 25-record pages client-side | Must Have | Previous / Next at minimum |
| FR-19 | Results table columns: Name, Spouse, City, State, Member Since (joinDate), Membership Type, Membership Status, Actions | Must Have | Empty fields show "—"; Spouse column shows `-NA-` when the member has no spouse on file |
...
| FR-21 | Results ordered by full name ascending | Should Have | Alphabetical UX expectation |
...
| NFR-02 | No PII beyond the seven result columns | Email, phone must not appear in API response (including spouse email); `memberId` only appears in the Send Message link href, never as a visible table cell | Response shape is a strict DTO, not a raw Prisma `Member`/`FamilyMember` row — spouse data is reduced to `fullName` only |
...
- [ ] Search by name returns members of any status whose own fullName contains the term
- [ ] Search by name also returns members whose spouse's fullName contains the term
- [ ] Search by name does NOT return members solely because a child's or other family member's name matches
- [ ] Search by city returns members of any status whose address.city contains the term
- [ ] Combined name + state filter returns the AND intersection
...
- [ ] Spouse column shows `-NA-` for members with no spouse on file
...
- [ ] `GET /api/members/search?name=ab` (2 chars) returns 400
- [ ] Response body contains no email or phone fields (member or spouse)
...
| Too-short input | Active member, name = "Na" | Click Search | "Minimum 3 characters" error |
| State dropdown swap | Active member, country = USA | Select Canada | State dropdown replaces with Canadian provinces |
| Name search — finds expired member | Active member; DB has expired member "Nayak" | name="Nayak" | Expired member row appears (any status in results) |
| Name search — matches partial/last-name-only | Active member; DB has member "John Smith" | name="Smith" | "John Smith" row appears |
| Spouse name search | Active member; DB has member "John Smith" whose spouse FamilyMember is "Jane Smith" | name="Jane" | "John Smith" row appears, Spouse column shows "Jane Smith" |
| Child name excluded | Active member; DB has member "John Smith" whose child FamilyMember is "Timmy Smith" | name="Timmy" | No row appears for "John Smith" |
| No spouse on file | Active member; DB has member "Mary Doe" with no spouse FamilyMember | name="Mary Doe" | Row appears with Spouse column showing "-NA-" |
...
| No results | Active member; no match | name="Zzzzz" | "No results found" |
| Pagination | 60 members match | Page 1 loaded | Shows rows 1–25, total "60 results", Next enabled |
| 1 000 cap hit | 1 200 members match | Search returns | Total shown as 1 000, cap warning displayed |
| API — no token | — | GET `/api/members/search?name=Nay` | 401 JSON |
| API — expired caller | Expired member's token | GET `/api/members/search?name=Nay` | 403 JSON |
| API — too-short param | Active token | GET `/api/members/search?name=ab` | 400 JSON |
...
| `apps/web/lib/validation/member.schema.ts` | **Modify** | Add `MemberSearchQuerySchema` (single `name` field with min-3-char refinement, country/state enum), `MemberSearchResultSchema` (incl. `name` and `spouseName`), and `MemberSearchResponseSchema` |
...
|---|----------|--------|--------|
| OQ-1 | Is `fullName` the only name field, or does `profileData` JSON contain separate `firstName`/`lastName` keys from the registration flow? | **Resolved** | `fullName` is the only stored name field; the search uses a single substring match against it (and against the spouse `FamilyMember.fullName`), covering full, partial, first-only, and last-only name queries without needing separate keys. |
