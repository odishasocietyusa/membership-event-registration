...
import type { ProviderPublic } from '@/lib/services/service-provider-service'
...
export default function EditForm({ provider }: { provider: ProviderPublic }) {
  const router = useRouter()
  const [bio, setBio] = useState(provider.bio)
  const [specializations, setSpecializations] = useState<string[]>(provider.specializations)
  const [onlineClasses, setOnlineClasses] = useState(provider.onlineClasses)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/services/${provider.id}`, {
        method: 'PATCH',
  ...
  async function handleDelete() {
    if (!confirm('Delete your service profile? This cannot be undone.')) return
    setDeleting(true)
